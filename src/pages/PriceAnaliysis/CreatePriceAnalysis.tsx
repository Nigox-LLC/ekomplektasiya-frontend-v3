...
//                   <th className="border-b border-slate-200 px-4 py-3">Kiruvmchi №</th>
//                   <th className="border-b border-slate-200 px-4 py-3">Chiquvchi №</th>
//                   <th className="border-b border-slate-200 px-4 py-3">Tovar</th>
//                   <th className="border-b border-slate-200 px-4 py-3">Tovar turi</th>
//                   <th className="border-b border-slate-200 px-4 py-3">Model</th>
...
import { useNavigate } from "react-router";
import { axiosAPI } from "@/service/axiosAPI"; // ⚠️ Yo‘lni o‘z loyihangizga moslang!

// Tovar interfeysi (API javobiga mos)
interface Product {
  id: number;
  product_name: string;
...
// Asosiy jadvalda ko'rsatiladigan tanlangan tovar (qo'shimcha maydonlar bilan)
interface SelectedProduct extends Product {
  selectedQuantity: number; // foydalanuvchi kiritgan yangi miqdor
}
...
const CreatePriceAnalysis: React.FC = () => {
  ...
  // Modallarni ochish state'lari
  const [chooseProductsOpen, setChooseProductsOpen] = useState(false);
  const [chooseOffersOpen, setChooseOffersOpen] = useState(false);
  const [uploadOfferOpen, setUploadOfferOpen] = useState(false);
  const [chooseEmployeesOpen, setChooseEmployeesOpen] = useState(false);

  // Tovarlar (modal uchun) state'lari
  ...
  // Tovarlarni yuklash funksiyasi (paginated)
  const fetchProducts = async (url: string) => {
    ...
    try {
      ...
      // Tanlovlarni tozalash (yangi sahifada)
  ...
  // Tanlangan tovarlarni asosiy jadvalga qo'shish
  const handleAddSelectedProducts = () => {
    const selectedProducts: SelectedProduct[] = [];
    selectedProductsMap.forEach((quantity, productId) => {
    ...
    // Mavjudlariga qo'shish (takrorlanmasligi uchun id bo'yicha filtr)
  ...
  const handleUploadSubmit = async () => {
    ...
    formData.append("date", uploadForm.date);  // backend ISO format kutadi: "YYYY-MM-DD"
    ...
    if (commercialFile) formData.append("file", commercialFile); // "file" – tijoriy taklif fayli

    setUploading(true);
    setUploadError(null);
    try {
      await axiosAPI.post("/document/analysis/commercial/", formData, {
        headers: { "Content-Type": "multipart/form-data" },
      });
      // Muvaffaqiyatli yuklandi – modalni yopish va formani tozalash
      ...
      // Agar xohlasangiz, shu yerda tijoriy takliflar ro‘yxatini qayta yuklash mumkin
    } catch (err: any) {
      // setUploadError(err.response?.data?.message || "Yukljjjjashda xatolik yuz berdi");
    } finally {
      setUploading(false);
    }
  };
  ...
  // Modal ichidagi checkbox tanlanganda
  const toggleProductSelection = (productId: number, checked: boolean) => {
    setSelectedProductsMap(prev => {
      const newMap = new Map(prev);
      if (checked) {
        // Standart miqdor sifatida price_analysis_quantity yoki 1
        const product = products.find(p => p.id === productId);
  ...
  // Modal ichidagi miqdor o'zgartirilganda
  ...
  const toggleSelectAll = (checked: boolean) => {
    if (checked) {
      ...
      products.forEach(p => {
        newMap.set(p.id, p.price_analysis_quantity || 1);
      });
    ...
    <div className="min-h-screen bg-slate-100 p-4 md:p-6">
      ...
      <div className="mx-auto flex w-full max-w-7xl flex-col overflow-hidden rounded-xl bg-white shadow-2xl">
        ...
        <div className="flex-1 overflow-auto bg-slate-50 p-4 md:p-5">
          ...
          {/* Qadam 2: Tijoriy takliflar (avvalgidek) */}
          {createStep === 2 && (
            <Card className="p-4">
              <div className="flex flex-wrap items-center gap-2">
                <Btn variant="secondary" onClick={() => setUploadOfferOpen(true)}>
          ...
          {/* Qadam 3: Imzolovchilar (avvalgidek) */}
          {createStep === 3 && (
            <Card className="p-4">
              <div className="flex flex-col gap-3 md:flex-row md:items-center md:justify-between">
                <div className="relative w-full md:w-96">
          ...
          {/* Qadam 4: PDF Preview (avvalgidek) */}
          {createStep === 4 && (
            <Card className="overflow-hidden">
              <div className="flex items-center justify-between border-b border-slate-200 bg-white px-4 py-3">
                <div className="text-sm font-semibold text-slate-800">Narx tahlili fayl</div>
      ...
      {/* Tovarlarni tanlash modali (API bilan) */}
      <ModalShell open={chooseProductsOpen} title="Tovarlarni tanlang" onClose={handleCloseProductsModal} size="xl">
        <div className="relative">
          ...
          {productsLoading && (
            <div className="mt-4 text-center py-8 text-slate-500">Yuklanmoqda...</div>
          )}
          {productsError && (
            <div className="mt-4 text-center py-8 text-red-500">Xatolik: {productsError}</div>
          )}
            ...
            <>
              ...
              <div className="mt-4 flex flex-col gap-2 border-t border-slate-200 pt-3 text-sm text-slate-600 md:flex-row md:items-center md:justify-between">
                ...
                <div className="flex items-center gap-2">
                  <Btn
                    variant="secondary"
                    disabled={!productsPrevUrl}
                    onClick={() => productsPrevUrl && fetchProducts(productsPrevUrl)}
                  >
                    Oldingi
                  </Btn>
                  <Btn
                    ...
                    onClick={() => productsNextUrl && fetchProducts(productsNextUrl)}
                  >
                    Keyingi
                  </Btn>
                </div>
      ...
      {/* Qolgan modallar (chooseOffersOpen, uploadOfferOpen, chooseEmployeesOpen) – xuddi avvalgidek qoladi, ularni ham API bilan to'ldirish mumkin */}
      <ModalShell open={chooseOffersOpen} title="Mavjud tijoriy takliflar" onClose={() => setChooseOffersOpen(false)} size="xl">
        <div className="flex items-center justify-between gap-2">
          <Btn variant="secondary" onClick={() => { }}><RefreshCw className="h-4 w-4" />Yangilash</Btn>
          <div className="relative w-72">
              ...
              <div className="flex items-center gap-2 rounded-md border border-slate-200 bg-white px-3 py-2 text-sm">
                ...
                <label className="cursor-pointer text-blue-600 hover:underline">
                  ...
                  <input
                    type="file"
                    className="hidden"
                    onChange={(e) => handleFileChange(e, setOrgInfoFile)}
                  />
                </label>
                {orgInfoFile && (
                  <button
                    onClick={() => setOrgInfoFile(null)}
              ...
              <div className="flex items-center gap-2 rounded-md border border-slate-200 bg-white px-3 py-2 text-sm">
                ...
                <label className="cursor-pointer text-blue-600 hover:underline">
                  ...
                  <input
                    type="file"
                    className="hidden"
                    onChange={(e) => handleFileChange(e, setStatFile)}
                  />
                </label>
                {statFile && (
                  <button
              ...
              <div className="flex items-center gap-2 rounded-md border border-slate-200 bg-white px-3 py-2 text-sm">
                ...
                <label className="cursor-pointer text-blue-600 hover:underline">
                  ...
                  <input
                    ...
                    className="hidden"
                    required
                    onChange={(e) => handleFileChange(e, setCommercialFile)}
                  />
                </label>
                {commercialFile && (
                  <button
