...
//   const [attachments, setAttachments] = useState<Record<number, { productId: number; price: number }[]>>({});
...
//   const handleAttachClick = (offerId: number) => {
//     setCurrentAttachOfferId(offerId);
//     // Mavjud biriktirilgan narxlarni yuklash (agar oldin saqlangan bo‘lsa)
//     const existing = attachments[offerId] || [];
//     const pricesMap: Record<number, number> = {};
//     existing.forEach(item => {
//       pricesMap[item.productId] = item.price;
//     });
...
//                           <td className="border-b border-slate-200 px-4 py-3 text-sm text-slate-700">
//                             <button
//                               onClick={() => handleAttachClick(offer.id)}
//                               className="text-blue-600 hover:underline flex items-center gap-1"
//                             >
//                               <Link2 className="h-4 w-4" />
//                               {attachments[offer.id]?.length ? (
//                                 <span className="ml-1 rounded-full bg-blue-100 px-2 py-0.5 text-xs">
//                                   {attachments[offer.id].length}
//                                 </span>
//                               ) : (
//                                 "Biriktirish"
...
//       {/* MODAL: Tovarlarga biriktirish */}
//       <ModalShell open={attachModalOpen} title="Tovarlarni biriktirish" onClose={() => setAttachModalOpen(false)} size="lg">
//         <div className="space-y-4">
//           {mainSelectedProducts.length === 0 ? (
//             <div className="text-center py-8 text-slate-500">
//               Hali hech qanday tovar tanlanmagan. Avval tovarlarni tanlang.
//             </div>
//           ) : (
//             <>
...
//                       <th className="border-b px-4 py-3">Tovar</th>
//                       <th className="border-b px-4 py-3">Miqdor</th>
//                       <th className="border-b px-4 py-3">Narx</th>
//                       <th className="border-b px-4 py-3">Summa</th>
//                     </tr>
//                   </thead>
//                   <tbody>
//                     {mainSelectedProducts.map(product => {
//                       const price = currentPrices[product.id] || 0;
//                       const total = price * product.selectedQuantity;
//                       return (
//                         <tr key={product.id} className="hover:bg-slate-50">
//                           <td className="border-b px-4 py-3 text-sm">{product.product_name}</td>
//                           <td className="border-b px-4 py-3 text-sm">{product.selectedQuantity}</td>
...
//                                 setCurrentPrices(prev => ({ ...prev, [product.id]: val }));
//                               }}
//                             />
//                           </td>
...
//                   .reduce((sum, [productId, price]) => {
//                     const product = mainSelectedProducts.find(p => p.id === Number(productId));
//                     return sum + (price * (product?.selectedQuantity || 0));
//                   }, 0)
...
//             <Btn variant="secondary" onClick={() => setAttachModalOpen(false)}>
//               Bekor qilish
//             </Btn>
//             <Btn
//               variant="primary"
//               onClick={() => {
//                 if (currentAttachOfferId) {
//                   // Faqat narxi 0 dan katta bo‘lgan tovarlarni saqlaymiz
//                   const attached = Object.entries(currentPrices)
...
//                   setAttachments(prev => ({
//                     ...prev,
//                     [currentAttachOfferId]: attached,
//                   }));
//                   setAttachModalOpen(false);
//                 }
//               }}
//               disabled={Object.values(currentPrices).filter(v => v > 0).length === 0}
//             >
//               Saqlash
//             </Btn>
//           </div>
