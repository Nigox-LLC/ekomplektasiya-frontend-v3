...
export function ExecutiveAction({
  ...
  const getCurrentHolder = () => {
    ...
    const lastStep = [...displaySteps]
      .reverse()
      .find((s) => s.status === "sent" || s.status === "pending");
    return lastStep
      ? lastStep.employee
      : displaySteps[displaySteps.length - 1]?.employee;
  };
        ...
        >
          <div className="space-y-0">
            {displaySteps.map((step, index) => {
              ...
              const isCurrent =
                step.employee === holderName &&
                (step.status === "sent" || step.status === "pending");
              ...
              return (
                <div key={step.id}>
                          ...
                          <div className="bg-gray-50 border border-gray-300 rounded-lg p-3 inline-flex items-center gap-3">
                            ...
                            <div>
                              <p className="text-xs font-medium text-gray-900">
                                {step.attachedFile.name}
                              </p>
                              {step.attachedFile.size && (
                                <p className="text-xs text-gray-500">
                                  {step.attachedFile.size}
                                </p>
                              )}
                  ...
                  <div
                    className={`flex items-center justify-between py-3 ${isCurrent ? "bg-blue-50 -mx-4 px-4 py-4 rounded-lg" : ""}`}
                  >
                    {/* Chap: Xodim */}
                    <div className="flex items-center gap-3">
                      <div
                        className={`flex-shrink-0 size-10 rounded-full flex items-center justify-center ${isCurrent ? "bg-blue-500 animate-pulse ring-4 ring-blue-300" : "bg-blue-500"}`}
                      >
                        <User className="size-5 text-white" />
                      </div>
                      <div>
                        <p
                          className={`text-sm font-bold ${isCurrent ? "text-blue-900" : "text-gray-900"}`}
                        >
                          {step.employee}
                        </p>
                        {step.position && (
