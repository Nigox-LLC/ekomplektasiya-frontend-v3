...
import { type IjroStep } from "./ExecutiveAction";
      ...
      const getActionText = () => {
        switch (selectedStatus) {
          case "roziman":
            return "Kelishilgan";
          case "rozi-emasman":
            return "Kelishilmagan";
          case "qisman":
            return "Qisman kelishilgan";
          default:
            return "Kelishish";
        }
      };
    ...
    <div className="fixed inset-0 bg-black/50 z-50 flex items-center justify-center p-4">
      <div className="bg-white rounded-lg shadow-2xl w-full max-w-2xl overflow-hidden">
        ...
        <div className="p-6 space-y-6">
          <div>
            ...
            <div className="grid grid-cols-3 gap-4">
              {(["roziman", "rozi-emasman", "qisman"] as const).map(
                (status) => {
          ...
          {selectedStatus && (
            <div
              className={`p-4 rounded-lg border-2 ${getStatusConfig(selectedStatus)?.borderColor} ${getStatusConfig(selectedStatus)?.bgColor}`}
            >
              <div className="flex items-start gap-3">
                {(() => {
                  ...
                  return (
                    <Icon
                      className={`size-6 ${config.iconColor} flex-shrink-0 mt-0.5`}
                    />
                  );
                })()}
                <div>
                  <p
                    className={`text-sm font-semibold ${getStatusConfig(selectedStatus)?.textColor}`}
                  >
                    Siz "{getStatusConfig(selectedStatus)?.label}" holatini
                    tanladingiz
                  </p>
                  <p className="text-xs text-gray-600 mt-1">
                    {selectedStatus === "roziman" &&
                      "Hujjat bilan to'liq rozisiz."}
                    {selectedStatus === "rozi-emasman" &&
                      "Hujjat bilan rozi emasligingizni bildirdingiz."}
                    {selectedStatus === "qisman" &&
                      "Hujjat bilan qisman rozisiz. Izoh qoldirishingiz tavsiya etiladi."}
                  </p>
                </div>
              </div>
            </div>
        ...
        <div className="flex items-center justify-end gap-3 p-6 border-t border-gray-200 bg-gray-50">
          <Button variant="outlined" onClick={onClose} className="border-2">
            Bekor qilish
          </Button>
          <Button
