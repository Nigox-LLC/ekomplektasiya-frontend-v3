...
import { Shield, FileText, FolderGit2, X, Save, Check, AlertCircle, MapPin, Map, Building2, Briefcase } from 'lucide-react';
...
const UserPermissionsPage: React.FC = () => {
    ...
    useEffect(() => {
        const fetchData = async () => {
            try {
                ...
                setError(err.response?.data?.message || 'Ma\'lumotlarni yuklashda xatolik yuz berdi. Iltimos qayta urinib ko\'ring.');
            } finally {
                setLoading(false);
            }
        };
    ...
    if (error) return (
        <div className={styles.centerInfo}>
            ...
            <button onClick={() => navigate(-1)} className={`${styles.btnOutline} ${styles.actionButton} mt-4 mx-auto`}>
                Ortga qaytish
            </button>
        </div>
    );
    if (!user) return <div className={`${styles.centerInfo} ${styles.errorText}`}>User not found</div>;
                                        ...
                                        <th key={action} className={`${styles.th} ${styles.thCenter}`}>
                                            <div className={styles.thContent}>
                                                <input
                                                    ...
                                                    onChange={(e) => handleSelectColumn(action as any, e.target.checked)}
                                                />
                                ...
                                {groupedPermissions.map((group, index) => {
                                    return (
                                        <tr key={group.model} className={styles.row}>
                                            ...
                                            <td className={`${styles.td} ${styles.tdName}`}>{formatModelName(group.model)}</td>
                                            {['view', 'add', 'change', 'delete'].map((action) => {
